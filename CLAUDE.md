...
- **`docs/`** — Design documents, reference, examples. This folder contains no code, but is a context gold mine. Start with `system_design.md`.
- **`docs/dev_artifacts`** — implementation plans, research, notes
...
2. **`transform.rs`** — Source transformation: strips frontmatter, injects `__RNME_GROUP` (path-derived group key) and `__RNME_DIR` (absolute path of the RUNME.rs's directory) constants.
...
- **`TaskDef`** — Task metadata. The `func` field is `TaskFnKind`: either `Static(TaskFn)` (function pointer from `#[rnme::task]`, const-constructible for `inventory::submit!`) or `Dynamic(Arc<dyn Fn>)` (closure with captured state, from `InitContext::register_task()`). The `dir: &'static str` field carries the absolute path of the originating RUNME.rs (from `__RNME_DIR`) and drives default-cwd behavior on `ctx.spawn`; empty string means inherit the rnme process's cwd. `TaskDefRef(&'static TaskDef)` is the inventory submission target — keeps the static named so the typed shim can reference it directly.
...
Every RUNME.rs file must define `const __RNME_GROUP: &str = "...";` and `const __RNME_DIR: &str = "...";` — both are injected by the code generator during compilation. For standalone tests/examples, define them manually as `""` (empty `__RNME_DIR` makes `ctx.spawn` fall back to the rnme process's cwd, matching the old behavior).
