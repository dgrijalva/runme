...
//! (`runme-test-tasks`) are visible via `Registry::from_inventory()` when
//! the `__runme_link()` pattern is used.
//!
//! ## The problem
//!
...
//! `<crate>::__runme_link()` for every RUNME.rs crate in the workspace.
...
//! use runme::prelude::*;
//!
//! const __RUNME_GROUP: &str = "my_group";
//!
//! #[runme::task(desc = "A reusable task")]
...
// Required: force the linker to include runme-test-tasks' inventory sections.
// Without this call, the tasks from that crate would be silently dropped.
fn link_shared_crate() {
    runme_test_tasks::__runme_link();
}

// ============================================================
// Test 1: Tasks from the shared crate are visible after __runme_link()
// ============================================================
...
#[runme::task(desc = "A local task in the test binary")]
