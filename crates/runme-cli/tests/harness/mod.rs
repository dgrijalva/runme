//! CLI test harness for running the `runme` binary against fixture directories.
//!
//! Provides `run_runme()` for subprocess invocation, `CliOutput` for inspecting
...
/// Captured output from a `runme` invocation.
...
/// Run the `runme` binary with the given arguments, using `dir` as the working directory.
///
...
pub fn run_runme(dir: &Path, args: &[&str]) -> CliOutput {
    let bin = env!("CARGO_BIN_EXE_runme");
    let output = Command::new(bin)
        .args(args)
        .current_dir(dir)
        .output()
        .unwrap_or_else(|e| panic!("failed to run runme binary at {}: {}", bin, e));
...
#[runme::task(desc = "Echoes arguments")]
...
#[runme::task(desc = "Produces output")]
...
#[runme::task(desc = "Fails with default code")]
...
#[runme::task(desc = "Minimal success task")]
...
#[runme::task(desc = "A task in a subdirectory")]
