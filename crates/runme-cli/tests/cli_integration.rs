...
//! actual `runme` binary against them. Tests are grouped to reuse compiled
//! fixtures where possible, but each compilation still takes several seconds.
//!
//! Run with: `cargo test -p runme-cli --test cli_integration`
//! Or to include ignored (slow) tests: `cargo test -p runme-cli --test cli_integration -- --ignored`
...
/// that happens when `run_runme` is called). Subsequent tests reuse the same dir,
...
fn no_runme_found() {
    ...
    let out = harness::run_runme(empty.path(), &["--ui", "cli", "anything"]);
