...
- **JsonlParser** -- Scans for JSON objects/arrays at the start of the buffer. Uses brace/bracket depth tracking (skipping over quoted strings) to find the end of the record. Handles newline-delimited JSON, concatenated JSON (`{"a":1}{"b":2}`), and pretty-printed JSON spanning multiple lines. Bytes consumed includes the record and any trailing whitespace. Stateless — emits a record each time a complete JSON object/array is found, then starts fresh. At EOF, emits partial JSON as PlainText. Non-JSON input is rejected immediately.
