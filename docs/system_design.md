...
```
rnme --ui cli deploy --env staging --port 8080
...
All tasks work in all UI modes, controlled by `--ui` (or defaulting based on context). Every mode runs through the same engine — the UI is just a viewport over the same task graph and log store:
- **TUI mode** (`--ui tui`, default) — ratatui-based interface for interactive navigation, log viewing, multi-task management. No args launches the task picker.
...
rnme --ui cli deploy --env staging
```
...
- **Agent mode** (`--ui agent`) — quiet, structured output optimized for token efficiency. Final result reported as JSON or text.
