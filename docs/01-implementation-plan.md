...
- Stub `#[runme::task]` and `#[runme::init]` attribute macros in `runme-macros` using `syn` + `quote`
  - `#[task]` transforms an annotated fn into a registered task with metadata (including `group` field)
  - `#[init]` registers a per-file initialization hook via `inventory`
- Add `GroupDef` and `InitContext` types to the library
...
**Note:** Phase 1 originally included `#[runme::main]`, which has been replaced by `#[runme::init]`. The entry point (`fn main()`) is generated by the build system's runner crate, not by a macro in the RUNME.rs file. See `docs/build_system_design.md` for the full compilation model.
...
- Implement compilation pipeline (see `docs/build_system_design.md` for full spec):
...
- Dependency frontmatter: RUNME.rs files may need crates beyond `runme` itself. Support an optional frontmatter block for declaring additional dependencies:
  ...
  //! serde_json = "1"
  ```
  The compilation pipeline parses this and adds them to each crate's generated Cargo.toml alongside the auto-injected `runme` dependency.
- Shebang support: `#!/usr/bin/env runme` — the OS invokes `runme` with the file path, runme discovers the full tree, builds, and runs
- `runme init` — generate a starter RUNME.rs for the current project (auto-detect cargo/npm/etc.)
- `runme list` — discover tasks and print them (handled by the generated runner crate)
...
After each phase:
- Phase 1: A RUNME.rs example with `#[task]` compiles and runs, tasks execute, `group` field is populated
- Phase 2: `runme` discovers multiple RUNME.rs files, generates a workspace, builds, and runs tasks from any file in the tree
